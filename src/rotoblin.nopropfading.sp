...
static	const	Float:	FADE_AFTER_TANK_DEATH_TIME	= 5.0; // Time for props to fade away after tanks death
...
_NPF_OnPluginEnabled()
{
	...
	HookEvent("entity_killed", _NPF_TankKilled_Event);
...
_NPF_OnPluginDisabled()
{
	...
	UnhookEvent("entity_killed", _NPF_TankKilled_Event);
 ...
 * @noreturn
 */
public _NPF_TankKilled_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (IsPlayerTank(GetEventInt(event, "entindex_killed")))
		CreateTimer(1.0, _NPF_FindAnyTank, _, TIMER_FLAG_NO_MAPCHANGE);
}

public Action:_NPF_FindAnyTank(Handle:timer)
{
 ...
 * @noreturn
 */
public _NPF_SDKh_PropTakeDamage_Event(victim, attacker, inflictor, Float:damage, damagetype)
{
	if (((IsClientAndInGame(attacker) && GetClientTeam(attacker) == 3 && IsPlayerTank(attacker)) || // If attacker is tank
...
static FadeTankHitProps()
{
	...
	new iArraySize = GetArraySize(g_hTankHitPropArray);
...
{
	if (g_hPropArray == INVALID_HANDLE) return;

	new iArraySize = GetArraySize(g_hPropArray);
	if (!iArraySize) return;

	for (new i = 0; i < iArraySize; i++)
		SDKUnhook(GetArrayCell(g_hPropArray, i), SDKHook_OnTakeDamagePost, _NPF_SDKh_PropTakeDamage_Event);
...
static HookAllPropPhysics()
{
	...
	for (new entity = MaxClients + 1; entity <= MAX_EDICTS; entity++)
	{
		...
		SDKHook(entity, SDKHook_OnTakeDamagePost, _NPF_SDKh_PropTakeDamage_Event);
...
{
	if (entity == INVALID_ENT_REFERENCE || entity > MAX_EDICTS || !IsValidEntity(entity)) return false;
