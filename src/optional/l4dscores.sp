...
public Plugin:myinfo =
{
	...
	author = "Downtown1, raziEiL [disawar1]",
...
{
	CreateNative("L4DScores_GetCampaingScore", Native_GetCampaingScore);

	return APLRes_Success;
}

public Native_GetCampaingScore(Handle:plugin, numParams)
{
	return GetTeamCampaignScore(GetNativeCell(1));
}
...
public OnMapStart()
{
	...
	if(pendingNewMission || StrContains(mapname, "01_") != -1)
	{
		OnNewMission();
	}
	else
...
CalculateNextMapTeamPlacement()
{
	...
	* Is the game about to automatically swap teams on us?
	*/
	g_pendingSwapScores = false;
	if(GetTeamCampaignScore(L4D_TEAM_SURVIVORS) > GetTeamCampaignScore(L4D_TEAM_INFECTED))
	{
		g_pendingSwapScores = true;
	}
	...
	*/
	...
	for(i = 1; i < L4D_MAXCLIENTS_PLUS1; i++)
	{
		if(IsClientInGameHuman(i))
		{
			...
			team = (bNewMatch ? GetClientTeam(i) : GetClientTeamForNextMap(i, g_pendingSwapScores));

			DebugPrintToAll("Next map will place %N to %d (bNewMatch %d)", i, team, bNewMatch);
