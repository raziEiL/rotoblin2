...
// - improved wall kicking detection mechanism
// - fixed false detections
// - added block wallkick instead slay player
static  Handle:g_hTrickTimer[MAXPLAYERS+1], Float:g_fCvarPounceCrouchDelay, bool:g_bCvarPluginMode;

public OnPluginStart()
{
	CreateConVar("stop_wallkicking_version", PLUGIN_VERSION, "Stop Wall Kicking plugin version", FCVAR_REPLICATED|FCVAR_PLUGIN|FCVAR_NOTIFY);

	new Handle:hCvarPounceCrouchDelay = FindConVar("z_pounce_crouch_delay")
	new Handle:hCvarPluginState = CreateConVar("stop_wallkicking_enable", "1", "Enables/disables the Stop Wall Kicking plugin", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
public Action:OnPlayerRunCmd(client, &buttons)
{
	if (!g_bCvarPluginMode && buttons & IN_ATTACK){
		...
		if (buttons & IN_JUMP && IsPlayerHunter(client) && !GetEntProp(client, Prop_Send, "m_isAttemptingToPounce"))
...
public Action:WK_ev_PlayerJump(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsPlayerHunter(client) && GetClientButtons(client) & IN_JUMP && !GetEntProp(client, Prop_Send, "m_isAttemptingToPounce"))
		RunTrickChecking(client);
}
